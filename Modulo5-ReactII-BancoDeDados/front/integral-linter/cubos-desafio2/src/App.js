...
function App() {
  ...
  function handleMovieBuy(movie) {
    ...
    const movieInBasket = newMovies.find(({ title }) => title === movie.title);
  ...
  function handleMovieAdd(movieTitle) {
    const newMovies = [...moviesInBasket];
    const movieInBasket = newMovies.find(({ title }) => title === movieTitle);

    movieInBasket.count++;
    setMoviesInBasket(newMovies);
    const newPrice = getDiscountedPrice(movieInBasket.price + basketFinalPrice);
    setBasketFinalPrice(newPrice);
  }
  ...
  function handleMovieRemoval(movieTitle) {
    const newMovies = [...moviesInBasket];
    const movieInBasket = newMovies.find(({ title }) => title === movieTitle);
    const newPrice = getDiscountedPrice(basketFinalPrice - movieInBasket.price);
    setBasketFinalPrice(newPrice);

    movieInBasket.count--;
    if (movieInBasket.count === 0) {
      setMoviesInBasket(newMovies.filter(({ title }) => title !== movieTitle));
      return;
    }
  ...
  return (
    <div className="App">
      <Header setMoviesNameFilter={setMoviesNameFilter} />
