...
select sum(estoque) from farmacia where categoria is null and estoque > 0 GROUP BY categoria
...
9 - Faça uma query que retorna uma única coluna com a junção do nome do medicamento e a categoria entre parenteses de todos os 
registros em que a categoria não seja nula da tabela `farmacia`. Ex.: `Endocet (green)`.
select concat(medicamento, ' ', '(',  categoria, ')') from farmacia where categoria is not null

10 - Faça uma query que retorna uma única coluna com a junção do identificador e o nome do medicamento separado por um hífen e a categoria entre parenteses de todos os registros da tabela `farmacia`. 
...
select nome, idade, cast(cadastro as date) from usuarios where EXTRACT(YEAR FROM cast(cadastro as date)) = 2020
...
select nome, idade, email, age(cast(cadastro as date)) from usuarios where idade >= 80
